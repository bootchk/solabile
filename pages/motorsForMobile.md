...
This page is specific to electric motors used in art mobiles.

## Basics of electric motors

This section provides general context.

This describes rotating electric motors using electromagnetism.
Linear electric motors have a different construction,
not suited for art mobiles.
Some motors uses electrostatic force,
...
One end of the shaft protruding through a bearing
...
Commonly motors uses brushes and commutator rings
...
Brushless motors used electrical commutation.
...
This discusses unique requirements for typical uses of electric motors
in art mobiles.
...
it may be desireable to use long-life motors.
...
That is compatible with small solar cells, which are also usually low voltage.
A typical single solar cell produces 0.6V.
A series of small solar cells can produce as much as 5 V.
The electronics to boost the voltage of solar cells adds complexity to the design.
...
More common motors again use 6V.

### Choosing sizes for levels of motorized art mobiles

As you progress downwards in the usual art mobile,
each level requires less power.
To use a different size motor in each level adds complexity to the design.
...
a motor oriented with shaft up 
...
The axial bearing surfaces also suffer sticktion.
The radial bearing surfaces may suffer less sticktion.
...
the motors are usually pulsed so that stall current is not endured long
and heat buildup is not a problem.